...
export const protocols = {openai_responses:{decodeRequest:function(ctx){return {kind:"submit",model:ctx.body.value.phase === "final" ? "drifted-model" : ctx.model,requestBody:ctx.body.value};},renderFinal:function(){return {};}}};
...
`
	...
	require.NoError(t, err)
	body, ok := protocolContext.RouteRequestContext.Body.(map[string]any)
	require.True(t, ok)
	bodyValue, ok := body["value"].(map[string]any)
	require.True(t, ok)
	bodyValue["phase"] = "final"
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", nil)
